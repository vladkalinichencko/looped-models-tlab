...
Идея в том, что я могу эмбеддинг контролировать, а не просто наблюдать за ним. До манифолдов дело здесь даже не доходит: видно, что направление просто увеличивается, и я смотрю уже на преобразование, а не на фиксированные точки выхода. Отсюда мысль: вместо того чтобы думать про манифолд, подумать про то, что главной проблемой является сама норма, а её отсутствие тоже проблема. Что если я само обновление заставлю математически быть не таким большим.
...
Здесь \(R\) — общий core из четырёх Qwen3-блоков, \(W-W^\top\) кососимметрична по построению, \(g\) — скалярный гейт по \([h_t,e]\), а \(\gamma\) — тот самый скалярный люфт. Исходный embedding \(e\) входит только в гейт; concat-adapter, как у Huginn, не используется, потому что смысл варианта в голой композиции с ограниченной нормой. Состояние \(h_t\) имеет shape `(batch, 512, 384)` и стартует с выхода embedding. Train и eval идут на 16 циклах, gradient проходит через все 16 повторов, loss — final next-token cross-entropy без auxiliary членов.
...
В текущей реализации контроллер — это GRU, который получает исходный embedding и историю предыдущих действий. На обучении oracle выбирает блок с минимальным token CE, рекуррентное состояние следует этому teacher-forced маршруту, а контроллер обучается отдельным CE к oracle action. Блоки обучаются суммой своего token CE и contrastive repulsion между выходами; на инференсе softmax смешивает четыре proposals. Точка стыковки двух идей в том, чтобы контроллер смешивал не полные proposals, а приращения с ограниченной по построению нормой.
...
| Контроллер | 9 450 500 | 49 995 776 | 16 | 64 | 10.2375 | 27 931.8 |
...
Антисимметричный переход проигрывает обоим, 231.78 против 78.70 и 88.07. Его кривая идёт отдельным пучком заметно выше остальных на всём бюджете. Это отрицательный результат основной идеи, и объясняю я его так. Ограничение нормы перехода сделало ровно то, что обещало: состояние перестало взрываться. Но качество от этого не выросло, а упало. Гипотеза про рост нормы подтвердилась в своей диагностической части, рост действительно есть и действительно подавляется, а терапевтическая часть не подтвердилась. Кососимметричная матрица не меняет длину вектора, и вместе с этим она отнимает у перехода слишком много: за 16 повторов модель не успевает сделать содержательную работу, которую обычный Qwen-блок делает за один.
...
Разбирая, почему антисимметричный вариант так отстал, я посмотрел на приращения состояния по циклам и нашёл настоящую причину. Она не в том, что кососимметричная матрица бедна.

| цикл | норма состояния | норма приращения | token loss |
|---:|---:|---:|---:|
...
Я проверял entropy, \(JS(p_t,p_{t+1})\), \(\|J_{z,t}\Delta h_t\|\) и Fisher length \(\Delta h_t^\top F_h\Delta h_t\) как предикторы фактической пользы следующего цикла \(L_t-L_{t+1}\). Среднее улучшение token loss на первом цикле равно 4.414, а на циклах 9–16 практически нулю. Корреляция JS с этой пользой падает с 0.642 на первом шаге до \(-0.101\) на последнем, а Fisher и \(\|J\Delta h\|\) на первом шаге дают отрицательные корреляции \(-0.559\) и \(-0.533\), то есть большая чувствительность здесь не означает полезное изменение.

По итогу диагностика показала, что все эти метрики, которые я считаю неопределённостью, являются неуверенностью модели, и пока что нет широко известных и работающих способов сделать эту идею действительно масштабируемой. Зависимость меняет знак с глубиной, поэтому единый score с фиксированным порогом не обоснован, и ранний выход в итоговый метод я не вводил. Чтобы вообще понять, есть ли здесь верхний предел для обученная политика, нужна при равном объёме вычислений кривая Oracle по известному будущему \(L_t-L_{t+1}\); её у меня нет.

Диагностика: [страница прогонов](report.html), прогон `cycle-probe-100k-r16`.

### Калибровка затухания по нулевому радиальному дрейфу

Я взял обученный вариант с \(\gamma=0\) и подобрал \(\gamma=0.196165\) так, чтобы на одной отложенных траектории среднее \(\left(\|h_{t+1}\|^2-\|h_t\|^2\right)/\|h_t\|^2\) было равно нулю, после чего обучил новую модель с нуля с этим начальным значением; к лучшему чекпойнт оно стало 0.192953. Калибровка выполнила ровно свою узкую задачу и ничего больше. Без затухания норма растёт с 0.508 до 5.340, средний squared-norm drift равен 0.354, selection CE равен 8.400; с затуханием норма идёт с 0.394 до 0.357, drift равен \(-0.029\), а selection CE ухудшается до 8.530. В сохранённых координатах общей PCA калиброванная модель никуда дальше не улетает: максимальное перемещение токена от первого до последнего цикла равно 0.112 против 1.118 без калибровки.
...
Значит, дело не в нормах и не в геометрии, а в том, что обучение и инференс идут по разным маршрутам. У Huginn норма тоже растёт, но путь один и тот же, и он работает. Чтобы идея со слоями как токенами заработала, контроллер должен получать сигнал от того же прохода, который потом используется на инференсе, а не имитировать оракула отдельной ошибкой. Мне эта идея по-прежнему кажется интересной, и я был бы рад довести её до рабочего вида на стажировке.
